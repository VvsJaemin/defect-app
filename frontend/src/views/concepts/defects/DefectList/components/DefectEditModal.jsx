...
const DefectEditModal = ({ isOpen, onClose, defectId, onSaveSuccess }) => {
    const { user } = useAuth()
    ...
    const { data, isLoading, error } = useSWR(
        defectId && isOpen ? `/defects/read/${defectId}` : null,
    ...
    const handleSave = async () => {
        try {
            ...
            // 서버에 결함 수정 요청
            const response = await ApiService.put('/defects/modify-defects', formDataToSend, {
                headers: {
            ...
            if (defectId) {
                mutate(`/defects/read/${defectId}`)
            }

            // 성공 콜백 호출 (부모 컴포넌트에서 목록 새로고침 등을 위해)
            if (onSaveSuccess) {
                onSaveSuccess()
            }
    ...
    return (
        <Dialog
            ...
            closable={false}
        >
            <div className="h-full flex flex-col" style={{ maxHeight: '95vh' }}>
                <div className="flex-shrink-0 px-6 py-4 border-b border-gray-200 bg-white">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl font-bold text-gray-900">결함 수정</h2>
                    ...
                    {error ? (
                        ...
                        <Card className="w-full">
                            <div className="flex flex-col xl:justify-between h-full 2xl:min-w-[360px] mx-auto">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-7 mt-4">
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                <Upload
                                                    ...
                                                    uploadLimit={Math.max(0, 3 - existingFiles.length)}
