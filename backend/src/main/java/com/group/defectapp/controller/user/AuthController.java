...
public class AuthController {
     ...
     * 
     * @param loginRequestDto 로그인 요청 정보 (userId, password)
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequestDto loginRequestDto, HttpServletRequest request) {
        ...
        User user;
        try {
            user = userService.findByUserId(loginRequestDto.getUserId());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(LOGIN_FAIL_MSG);
        }
